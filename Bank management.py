...
class BankSystem:
    def __init__(self, master):
        self.master = master
        self.master.title("Bank Server System")
        ...
        self.dob_label = Label(self.create_account_frame, text="Dob:", font=('Arial', 12), bg='#F0F0F0')
        ...
        # Labels        
        label_style = {"fg": "blue", "font": ("Calibri", 16)}
        ...
        self.dob_label2.grid(row=1, column=1, padx=10, pady=10)
    ...
    def create_account(self):
        ...
        user_data = {'name': name, 'dob': dob, 'salary': salary, 'pin': pin, 'balance': 0, 'transaction_log':[]}

        # Add the user's data to the users dictionary
        self.users[pin] = user_data
        
        #  input valid or no
        if not name or not dob or not salary or not pin:
            messagebox.showerror("Error", "All fields are required!")
            return
        if not dob.isdigit():
            messagebox.showerror("Error", "Dob must be a number!")
            return
        ...
        self.dob_label2.config(text="Dob: " + self.dob)
    ...
    def login(self, event=None):
        ...
        if pin in self.users and self.users[pin]['name'] == name:
            ...
            self.dob_label2['text'] = f"Dob: {self.current_user_data['dob']}"
